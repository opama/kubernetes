...
var _ = framework.KubeDescribe("ReplicaSetMali", func() {
	f := framework.NewDefaultFramework("replicasetmali")

	It("should serve a basic image on each replica with a public image [mali]", func() {
		ReplicaSetMaLiServeImageOrFail(f, "basic", "gcr.io/google_containers/serve_hostname:v1.4")
	})

	PIt("should serve a basic image on each replica with a private image", func() {
		// requires private images
		framework.SkipUnlessProviderIs("gce", "gke")

		ReplicaSetMaLiServeImageOrFail(f, "private", "b.gcr.io/k8s_authenticated_test/serve_hostname:v1.4")
	})
})
...
func ReplicaSetMaLiServeImageOrFail(f *framework.Framework, test string, image string) {
